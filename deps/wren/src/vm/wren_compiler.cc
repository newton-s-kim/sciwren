...
static void classDefinition(Compiler* compiler, bool isForeign)
{
    ...
    Value classNameString =
        wrenNewStringLength(compiler->parser->vm, compiler->parser->previous.start, compiler->parser->previous.length);
...
static void import(Compiler* compiler)
{
    ...
    do {
        ...
        int sourceVariableConstant = addConstant(
            compiler, wrenNewStringLength(compiler->parser->vm, sourceVariableToken.start, sourceVariableToken.length));
