...
//#define USE_SYSTEM_INPUT

#ifdef USE_SYSTEM_INPUT
#include "read_line.h"
#endif // USE_SYSTEM_INPUT
...
static const char* resolveModule(WrenVM* vm, const char* importer, const char* module)
{
...
{
    WrenForeignMethodFn method = bindBuiltInForeignMethod(vm, module, className, isStatic, signature);
    if (method != NULL)
        return method;
...
{
    WrenForeignClassMethods methods = bindBuiltInForeignClass(vm, module, className);
...
WrenInterpretResult runRepl()
{
    ...
    while (true) {
        input = rl.read("wren/> ");
        result = wrenInterpret(vm, "<repl>", input.c_str());
    }
#else  // USE_SYSTEM_INPUT
    printf("\\\\/\"-\n");
    printf(" \\_/   wren v%s\n", WREN_VERSION_STRING);
