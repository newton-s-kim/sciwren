...
void fileOpen(WrenVM* vm)
{
    ...
    uv_fs_open(getLoop(), request, path, mapFileFlags(flags), S_IRUSR | S_IWUSR, fileOpenCallback);
}
...
void fileReadBytes(WrenVM* vm)
{
    ...
    uv_fs_read(getLoop(), request, fd, &data->buffer, 1, offset, fileReadBytesCallback);
}
...
void fileWriteBytes(WrenVM* vm)
{
    ...
    uv_fs_write(getLoop(), request, fd, &data->buffer, 1, offset, fileWriteBytesCallback);
}
...
static void stdinReadCallback(uv_stream_t* stream, ssize_t numRead, const uv_buf_t* buffer)
{
    WrenVM* vm = getVM();
